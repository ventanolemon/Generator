...
class VoiceRecorder(QWidget):
    ...
    (см. `core.answers._recording_path`). Проверка идёт на этой же машине
    в этом же процессе, поэтому путь тут законен; по сети такое передавать
    нельзя, и веб-половине понадобится свой способ.
    """
