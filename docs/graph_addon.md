...
На этом линейная алгебра (Фаза 3e) покрывает модуль линал: матрицы (det/inv/
rank/…), системы и операторы (rref/eigen/nullspace/linsolve), вектор-геометрия
с прямыми и плоскостями. Всего 27 узлов категории `linalg`; вектор = матрица n×1.
Это база и для дальнейших задач линейной алгебры (квадратичные формы, базисы,
ЖНФ — добавляются узлами того же образца).
