...
Остались известными (не сделаны — приоритет за пользователем): узел-сторож
`guard` (BOOL→retry для rejection sampling сложных заданий) и полировка
`prefix`/авто-переменная diff. Тесты: `tests/test_graph_choice.py` (22).
