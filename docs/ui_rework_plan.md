...
- **C** ✅ — клиент контура + неблокирующий поллер; мастер «Генератор через
  ...
  Экран приёмки (S6) визуально приведён к Fable-макету `contour_review.html`:
  шапка-карточка (описание + чипы типа/статуса), карточка критика с
  вердикт-бейджем (принять/доработать/отклонить — `badge-ok`/`chip`/
  `badge-error`, не `badge-warn`), метром уверенности (QProgressBar) и
  warn-флагами проб, компактный таймлайн раундов, первичная кнопка «Принять
  задание». Инварианты тестов сохранены (превью с `preview_seed`, флаги
  `badge-warn`, текст «Критик:» + сырая уверенность).
...
### Блокер: единая идентичность — СНЯТ (разрешился в другую сторону)
Блокер был: десктоп знает пользователя как строку-логин, сервер — как число,
и фильтр `owned_by` по логину против числовых владельцев показал бы учителю
только встроенные предметы. Рекомендация этого раздела была «завести в сессии
числовой server-assigned `user_id`».

Система пошла **противоположным** путём, и это оказалось верно: канонический
идентификатор везде — **логин**. Миграция 003 увела групповые FK на логин,
`core/session.py` объявляет логин каноническим (`user_id` = `login`), синк и
контур шлют `X-User-Id` строкой, `owner_user_id` — TEXT. Числовой `users.id`
остался внутренним ключом сервера, наружу не торчит. Никакой ремап локальных
id при первом sync не понадобился.

Практический вывод: новые права ключуются логином (см.
`docs/subject_grants.md`, таблица `subject_grants`).

### Где живёт access-control
Разграничение — **серверный pull-scope**: `/sync/pull` отдаёт учителю только
доступные предметы. Но у этого есть две границы, выясненные при проектировании
выдач (подробно — `docs/subject_grants.md`):

1. **Встроенные предметы им не закрыть.** `bootstrap.sync_database`
   пересоздаёт их из `CODE_GENERATORS` на каждом старте, что бы сервер ни
   отдал. Для них ограничение — локальный фильтр витрины, то есть UI-уровень.
   Честный withhold возможен только для серверного контента.
2. **Курсорный pull не переносит изменение прав.** Диф по `row_version` не
   породит события ни при выдаче (версия старая, курсор её прошёл), ни при
   отзыве (версия не менялась). Нужна scope-эпоха с resync и sweep.

Локальный фильтр `owned_by` по-прежнему лишь удобство витрины, не защита.
...
6. **Домашки (assignments: партиция→группа)** ✅ — сервер `/assignments`
   ...
   Тесты: `test_assignments_client` (8), `test_homework_window` (9).
7. **Учёт выполнения домашки** ✅ — «сдал» = участник группы имеет верную
   попытку по выданной партиции (доменно-верный сигнал, без прокидывания
   `attempts.assignment_id` через UI решения — это остаётся будущим
   уточнением). Сервер: `assignment_completion_counts`/`assignment_progress`
   (Repository), `list_teaching` обогащается «сдали X из Y», `progress`
...
9. **Выдача предметов преподавателям** — десктоп ✅, сервер ⏳.
   Контракт и обоснование решений: `docs/subject_grants.md`.
   - Сделано на десктопе: `GrantsSnapshot` + таблицы `SubjectGrants`/
     `GrantsMeta` (ключ — логин) и `list_subjects(apply_grants=…)`;
     `core/grants/GrantsClient` (свои выдачи + админская матрица, транспорт
     как у `AdminClient`); scope-эпоха в синке — `resync` со *sweep*'ом
     (`SyncClient._sweep`, `SyncStore.get/set_scope_version`); вкладка
     «Предметы преподавателям» в `AdminWindow` (матрица с чекбоксами,
     черновик + последовательное сохранение строк, переключатель режима
     умолчания); снимок обновляется вместе с синком (`_SyncWorker(after=…)`).
     Тесты: `test_subject_grants` (39).
   - Ждёт сервера (GenerationWeb): таблица `subject_grants`, счётчик
     `scope_version`, скоуп и `resync` в `/sync/pull`, эндпоинты
     `/subjects/grants/mine` и `/admin/subject-grants*`. До их появления
     клиент ведёт себя как «снимка нет» — витрина не ограничивается.
   - **Границы честности** (из `docs/subject_grants.md`): встроенные
     предметы прячутся только на уровне витрины — их пересоздаёт
     `bootstrap.sync_database` из `CODE_GENERATORS`, а генераторы едут в
     клиенте. Sweep поэтому НЕ трогает встроенные, не трогает
     неподтверждённую сервером локальную работу и пишет напрямую мимо
     `sync_listener` — иначе потеря доступа ушла бы tombstone'ом и удалила
     бы предмет у всех.
