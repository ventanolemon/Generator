...
### Что разблокируется ПОСЛЕ подтверждения решения
- **owner-фильтр витрины** (`list_subjects(owned_by=session.login)`) —
  готовый однострочник в generator_window. НЕ включён: корректен только
  если сервер шлёт владельцев **строкой-логином** (иначе у учителя скрылись
  бы его же предметы). Ждёт серверной правки owner_user_id → login.
- Серверный pull-scope владения; атрибуция попыток к пользователю; группы.
