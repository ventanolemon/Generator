...
class MatrixSpectrumModel(Model):
    ...
    def build(self, rng, **params) -> Instance:
        ...
        n = cfg["size"]
        # `lamda`, а не `lambda`: второе — ключевое слово Python, и разбор
        # ответа на нём ломается целиком («Выражение не разобрано» на любой
        # ввод). Написание без `b` — собственная договорённость sympy ровно
        # для этого случая: печатается оно всё равно как λ. Поймано при
        # сборке задания «выпишите характеристический многочлен»: у него не
        # находилось ни одного принимаемого примера, то есть эталон не
        # проходил собственную проверку.
        lam = sp.Symbol("lamda")
