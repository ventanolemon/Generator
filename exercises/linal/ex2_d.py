...
def get_exercise():
    ...
    # В условии даются ровно те прямые, о которых говорит преамбула:
    # A1A3 и A2A3. Раньше печатались AB и AC — то есть вместо стороны
    # A2A3 студент получал сторону A1A2, и найти A3 по такому условию
    # НЕЛЬЗЯ: пересечение AB и AC это сама вершина A1. Ответ при этом
    # считался по A1A3 ∩ A2A3, то есть по данным, которых в условии не
    # было. Задание было нерешаемым в каждом выпуске.
    task += f"\nкоординаты точек A1: {a}, A2: {b}\n"
    task += f"уравнения A1A3: {ac.get_obch()}, A2A3: {bc.get_obch()}\n"
